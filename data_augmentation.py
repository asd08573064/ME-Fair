...
import numpy as np
from util.auto_augment import AutoAugment, Cutout

class ISIC2019_Augmentations():
    def __init__(self, is_training, image_size=256, input_size=512, model_name='VGG'):
        mdlParams = dict()
        ...
        # For VGG
        # if model_name == 'VGG':
        #     mdlParams['setMean'] = np.array([0.485, 0.456, 0.406])   
        #     mdlParams['setStd'] = np.array([0.229, 0.224, 0.225])
        # else:
        mdlParams['setMean'] = np.array([0.0, 0.0, 0.0])   
        mdlParams['setStd'] = np.array([1.0, 1.0, 1.0])
