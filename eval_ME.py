...
def early_exit_inference(x, model, model_name):
        outputs = []
        confidences = []
        output_id = 0
...
def sdn_test_early_exits_fairness(model, loader, device='cpu', model_name='', dataset_name='', eval_final=False):
    print(dataset_name)
    model.eval()
    gender_list = []
    label_list = []
    ...
    print("Current thredhold: {}".format(model.confidence_threshold))
    with torch.no_grad():
        for batch in tqdm(loader):
            ...
            bsz = b_y.shape[0]
            output, output_id, is_early = early_exit_inference(b_x, model, model_name)
            pred = None
...
def sdn_test_early_exits_fairness_utk(model, loader, device='cpu', model_name='', dataset_name='', eval_final=False):
    print(dataset_name)
    model.eval()
    gender_list = []
    label_list = []
...
def eval_sdn_celebA(model, one_batch_dataset, model_name, dataset_name=''):
    for threshold in [0.65, 0.7, 0.75, 0.775, 0.8, 0.85, 0.9, 0.95, 0.99, 0.999]:
        model.confidence_threshold = threshold
        print("Threshold: {}".format(threshold))
        sdn_test_early_exits_fairness(model, one_batch_dataset.test_loader, model_name=model_name, dataset_name=dataset_name)

def eval_sdn_utk(model, one_batch_dataset, model_name, dataset_name=''):
    for threshold in [0.65, 0.7, 0.75, 0.775, 0.8, 0.85, 0.9, 0.95, 0.99, 0.999]:
        model.confidence_threshold = threshold
        print("Threshold: {}".format(threshold))
        sdn_test_early_exits_fairness_utk(model, one_batch_dataset.test_loader, model_name=model_name, dataset_name=dataset_name)
...
if __name__ == '__main__':
    ...
    model.cuda()
    remove_list = []
    one_batch_dataset = dataset_handler(args.dataset)(batch_size=1, fairness_attribute=args.fairness_attribute, target_attribute=args.target_attribute, remove_img=remove_list)

    if(args.dataset == 'celebA'):
        eval_sdn_celebA(model, one_batch_dataset, args.model_name, dataset_name='')
    else:
        eval_sdn_utk(model, one_batch_dataset, args.model_name, dataset_name='')
