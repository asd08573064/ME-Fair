...
def cnn_test_early_exits_fairness(model, loader, device='cpu', model_name='', dataset_name='', eval_final=False):
    print(dataset_name)
    model.eval()
    top1 = AverageMeter()
    groupAcc=[]
...
def cnn_train(model, data, epochs, optimizer, scheduler, device='cuda', tensor_board_path='', models_path='', sensitive_group_name_list=[]):
    for epoch in range(1, epochs):
        ...
        if epoch % 5 == 0:
            ...
            print('Start testing...')
            cnn_test_early_exits_fairness(model, data.test_loader, model_name='resnet18', dataset_name='utk')


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='train_cnn')
    parser.add_argument('--fairness_attribute', type=str, default='Race',
                    help='fairness_attribute')
    parser.add_argument('--target_attribute', type=str, default='Age',
                    help='')
    parser.add_argument('--training_title', type=str, default='20220916_cnn_utk_Age_Race',
                    help='')
    ...
    dataset = dataset_handler('utk')(batch_size=args.batch_size, fairness_attribute=args.fairness_attribute, target_attribute=args.target_attribute)
